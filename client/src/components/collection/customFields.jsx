...
function CustomFields({ collection, register, control, setValue }) {
    const getInputOptions = (key) => {
        const updatedKey = key.replace('Name', '')
        return {
            "customBoolFieldName": <input type="checkbox" {...register(`customFields.${updatedKey}`, { required: true })} onChange={(e) => setValue(`customFields.${updatedKey}`, e.target.checked)}/>,
